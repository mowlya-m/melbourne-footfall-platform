...
COORDS = {
    3:(-37.81101,144.96429,"Melbourne Central"), 5:(-37.81874,144.96787,"Princes Bridge"),
    4:(-37.81496,144.96660,"Town Hall (West)"), 6:(-37.81800,144.96700,"Flinders Street Station"),
    2:(-37.81357,144.96506,"Bourke St Mall (South)"), 1:(-37.81340,144.96520,"Bourke St Mall (North)"),
    9:(-37.81830,144.95270,"Southern Cross"), 13:(-37.81120,144.95560,"Flagstaff"),
    15:(-37.80970,144.96470,"State Library"), 18:(-37.81430,144.97260,"Collins Place"),
    23:(-37.81870,144.95430,"Spencer-Collins"), 27:(-37.80690,144.95770,"QV Market"),
    30:(-37.81000,144.97260,"Lonsdale-Spring"), 34:(-37.81560,144.97370,"Flinders-Spring"),
...
def main():
    ...
    for lid, g in facts.groupby("location_id"):
        if lid not in COORDS: continue
        lat,lon,name = COORDS[int(lid)]
        sensors.append({"id":int(lid),"name":name,"lat":lat,"lon":lon,
    ...
    for h in range(24):
        ...
        hourly["weekday"].append(int(wd or 0)); hourly["weekend"].append(int(we or 0))

    data={"sensors":sensors,"hourly":hourly,
        "forecast":{"from_hour":15,
            "actual":[hourly["weekday"][h] for h in range(9,16)],
            "predicted":[hourly["weekday"][h%24] for h in range(15,22)]},
        "meta":{"total_readings":int(facts.pedestrian_count.count()),
            "gold_facts":len(facts),"sensor_versions":int(facts.sensor_key.nunique()),
            "mape":"2.8%","generated":"live from Athena"}}
